...
OUTPUT_DIR=${OUTPUT_DIR:-"$ROOT/results/ncu_hierarchical_dataflow"}
...
profile() {
    ...
    "$NCU" --target-processes all --replay-mode kernel --cache-control none --clock-control base \
        --launch-count "$launch_count" --metrics "$METRICS" --page raw --csv --force-overwrite \
...
for word_bits in 32 64; do
    ...
    # Hybrid2D is a two-launch plan; HierarchicalDataflow is one cooperative launch.
    profile "hybrid2d_w${word_bits}_n20_b4" 2 "${common[@]}" --backend hybrid2d --compute-unit radix4
    profile "hierarchical_w${word_bits}_n20_b4" 1 "${common[@]}" --backend hierarchical-barrier \
...
python3 "$ROOT/scripts/analyze_hierarchical_dataflow_ncu.py" "$OUTPUT_DIR/summary.csv" \
    --csv "$OUTPUT_DIR/plan_totals.csv" --markdown "$OUTPUT_DIR/analysis.md"
printf 'HierarchicalDataflow NCU summary: %s\n' "$OUTPUT_DIR/summary.csv"
