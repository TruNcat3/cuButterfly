...
def write_markdown(path, rows, batch):
    lines = [
        ...
        "| Configuration | Kernels | Time us | DRAM read MiB | DRAM write MiB | Warp inst. | Bank conflicts | DRAM peak | Barrier stall | Scoreboard stall |",
        "|:--|--:|--:|--:|--:|--:|--:|--:|--:|--:|",
    ]
    for row in rows:
        lines.append(
            f"| {row['configuration']} | {row['kernel_count']} | {row['time_us']:.3f} | "
    ...
    lines += ["", "## Controlled Changes", "",
              ...
              f"{(fixed['dram_write_mib_vs_pre_vector'] - 1.0):+.1%}.",
              f"- Selecting the new physical mapping changes replay time by "
              f"{(selected['time_us_vs_pre_vector'] - 1.0):+.1%} relative to the pre-vector fixed point.", ""]
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines))
