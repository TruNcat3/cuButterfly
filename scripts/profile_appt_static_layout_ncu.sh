...
OUTPUT_DIR=${OUTPUT_DIR:-"$ROOT/results/ncu_appt_static_layout"}
...
for bits in 32 64; do
    ...
    else modulus=576460756061519873; resident=2; roles=11,9,2
    fi
    for batch in "${batches[@]}"; do
        if [[ $bits == 32 ]]; then
            (( batch >= 16 )) && static_roles=7,14,1 || static_roles=8,12,1
            if (( batch == 1 )); then roles=8,12,2
            elif (( batch >= 16 )); then roles=7,15,2
            else roles=7,14,2
            fi
        else
        ...
        appt=("${common[@]}" --backend hierarchical-dataflow --stage-partition 7,7,6
              ...
              --target-ctas-per-sm "$resident" --appt-role-weights "$roles")
        for fragment in 8 16 32; do
            profile "static_fw${fragment}_u${bits}_b${batch}" "${appt[@]}" \
                --output-order appt-static --appt-fragment-width "$fragment" \
                --appt-writer-tiles 1 --appt-role-weights "$static_roles"
            profile "natural_fw${fragment}_u${bits}_b${batch}" "${appt[@]}" \
...
python3 "$ROOT/scripts/analyze_appt_static_layout_ncu.py" \
    "$OUTPUT_DIR/summary.csv" --markdown "$OUTPUT_DIR/analysis.md"
printf 'APPT layout NCU analysis: %s\n' "$OUTPUT_DIR/analysis.md"
