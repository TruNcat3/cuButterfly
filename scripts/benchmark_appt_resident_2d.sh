...
OUTPUT_DIR=${OUTPUT_DIR:-"$ROOT/results/appt_resident_2d"}
TRIALS=${TRIALS:-2}
WARMUP=${WARMUP:-3}
REPEAT=${REPEAT:-20}
...
for bits in 32 64; do
    if [[ $bits == 32 ]]; then
        modulus=998244353; resident=3; role_space=$U32_ROLES
    else
        modulus=576460756061519873; resident=2; role_space=$U64_ROLES
    fi
    for batch in "${selected_batches[@]}"; do
        case "$bits:$batch" in
            32:1) fragment=8; writer_tiles=1; writer_roles=6,12,4 ;;
            32:4) fragment=8; writer_tiles=2; writer_roles=6,12,4 ;;
            32:16) fragment=16; writer_tiles=2; writer_roles=6,12,4 ;;
            64:1|64:4) fragment=8; writer_tiles=2; writer_roles=8,8,4 ;;
            64:16) fragment=16; writer_tiles=2; writer_roles=8,8,4 ;;
