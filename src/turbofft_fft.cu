...
void launch(Kernel kernel, const Complex32* input, Complex32* output, std::uint64_t transforms, cudaStream_t stream) {
    ...
    kernel<<<static_cast<unsigned int>(blocks), block_threads, shared_bytes, stream>>>(
...
void launch_turbofft_generated(std::uint32_t log_n, const Complex32* input, Complex32* output, std::uint64_t transforms,
                               std::uint64_t batch_distance, std::uint64_t element_stride, cudaStream_t stream) {
    ...
    switch (log_n) {
        case 7: launch<7>(fft_7, input, output, transforms, stream); return;
        case 8: launch<8>(fft_8, input, output, transforms, stream); return;
        case 9: launch<9>(fft_9, input, output, transforms, stream); return;
        case 10: launch<10>(fft_10, input, output, transforms, stream); return;
