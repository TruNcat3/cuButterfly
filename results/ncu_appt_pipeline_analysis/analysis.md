...
The online kernel has a `224.1 us` fixed term and a `450.0 us/transform` fitted steady cost. The mature v0.6 resident core needs `128.2 us/transform`, so the remaining steady-state gap is `3.51x`; launch or graph submission changes cannot remove it.

Online cross-fold execution improves the staged APPT steady cost by `1.184x`, but adds `102.8 us` of fill/drain cost. This produces `0.971x` at batch 1 and `1.113x` at batch 4, directly confirming that homogeneous load amortizes the online pipeline.

Relative to staged APPT, online execution reduces warp instructions by `55.5%` and integer instructions by `55.8%`, while registers fall from `116` to `80` per thread. The remaining physical bottleneck is the boundary/output layout: store sectors rise from `4.01x` to `4.50x`, DRAM utilization falls to `11.9%`, and active warps fall to `18.3%`.
